...
def build_support_vector(embedding: torch.Tensor, label_one_hot: torch.Tensor) -> torch.Tensor:
    """Builds the input vector for a support example.
    Args:
    ...
    """
    return torch.cat([embedding, label_one_hot, torch.zeros(1, device=embedding.device)])  # Append zero to indicate support example


def build_query_vector(embedding: torch.Tensor, ways: int) -> torch.Tensor:
    """Builds the input vector for a query example.
    Args:
        embedding: Tensor of shape (embedding_dim,)
        ways: Number of classes (ways)
    Returns:
        Tensor of shape (embedding_dim + ways + 1,)
    """
    return torch.cat(
...
def train_epoch(
    ...
    for _ in tqdm(range(config.episodes_per_epoch), desc="train episodes", leave=False):
        ...
        for idx in range(support_embeddings.shape[0]):  # Iterate over support set and update state. This part does a forward pass only.
        ...
        for idx in range(query_embeddings.shape[0]):  # Iterate over query set and compute loss. This part does a forward pass and backward pass.
            ...
            eta_values.append(float(outputs["eta"].item()))
            plastic_values.append(float(outputs["diagnostics"]["plastic_norm"].item()))
        episode_loss.backward()  # Backpropagate the loss
